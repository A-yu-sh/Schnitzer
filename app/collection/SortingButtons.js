...
const label = [
  { label: "Sort By", slug: "Sort By", Checkdisabled: true },
  { label: "Relevance", slug: "Relevance", Checkdisabled: false },
  { label: "Price: Low to High", slug: "Ascending", Checkdisabled: false },
  { label: "Price: High to Low", slug: "Descending", Checkdisabled: false },
];
...
const SortingButtons = () => {
  ...
  return (
    <div>
      <div className="flex justify-end mt-5 md:justify-end">
        <label className="flex  text-sm font-medium text-gray-900 dark:text-white">
          <select
            ...
            {label.map((e) => {
              return (
                <option key={e.slug} value={e.slug} disabled={e.Checkdisabled}>
                  {e.label}
                </option>
              );
            })}
